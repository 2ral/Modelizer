...
class Transformer(torch.nn.Module):
    def __init__(self,
        ...
        dropout = 0.0 if num_decoder_layers + num_decoder_layers == 2 else dropout
    ...
    def encode(self, src: torch.Tensor, src_mask: torch.Tensor):
        with torch.no_grad():
            encoder_output = self.transformer.encoder(self.positional_encoding(self.src_token_embedding(src)), src_mask)
    ...
    def beam_decode(self, trg: torch.Tensor, encoder_outputs: torch.Tensor, device=torch.device('cpu'), beam_size: int = 2,):
        decoder_output = self.decode(trg, encoder_outputs, device).transpose(0, 1)
